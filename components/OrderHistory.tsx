...
export default function OrderHistory() {
  ...
  const sendLink = async (event: React.FormEvent) => {
    ...
    const { error: authError } = await supabase.auth.signInWithOtp({ email, options: { emailRedirectTo: `${window.location.origin}/account/orders` } });
    setSending(false);
    ...
    <section className="min-h-[70vh] bg-[#0A0A0A] py-20">
      <div className="container-site max-w-xl">
        <div className="card-dark p-7 sm:p-10">
          ...
          <h1 className="text-3xl sm:text-4xl font-black text-white mb-4">Reorder without starting over.</h1>
          <p className="text-gray-400 leading-relaxed mb-8">Enter the work email used on your quote. We&apos;ll email a secure sign-in link—no password or account setup required.</p>
          {sent ? <div className="bg-[#39FF14]/10 border border-[#39FF14]/25 rounded-lg p-5"><div className="flex gap-3"><CheckCircle className="text-[#39FF14] shrink-0" /><div><p className="font-semibold text-white">Check your email</p><p className="text-sm text-gray-400 mt-1">Use the secure link we sent to {email}.</p></div></div></div> : <form onSubmit={sendLink} className="space-y-4"><div><label className="input-label">Work email</label><input required type="email" className="input-field" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@company.com" /></div><button disabled={sending} className="btn-primary w-full">{sending ? <><Loader2 size={16} className="animate-spin" /> Sending...</> : "Email my secure link"}</button></form>}
    ...
    <section className="min-h-[70vh] bg-[#0A0A0A] py-16">
      <div className="container-site">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-5 mb-10">
          <div><h1 className="text-4xl font-black text-white mb-2">Wire order history</h1><p className="text-gray-400">Signed in as {signedInEmail}</p></div>
        ...
        {!orders.length ? <div className="card-dark text-center py-14"><Package size={36} className="text-gray-600 mx-auto mb-4" /><h2 className="text-xl font-bold text-white mb-2">No wire requests yet</h2><p className="text-gray-400 mb-6">When you request a wire quote with this email, it will appear here.</p><Link href="/wire/order" className="btn-primary">Build a wire quote</Link></div> : <div className="space-y-5">{orders.map((order) => <article key={order.id} className="border border-white/10 rounded-xl bg-[#111] overflow-hidden">
          <div className="p-5 sm:p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/10">
            <div><p className="text-xs text-gray-500 font-mono">REQUEST {order.id.slice(0,8).toUpperCase()}</p><h2 className="text-lg font-bold text-white mt-1">{new Date(order.created_at).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}</h2><p className="text-sm text-gray-400 mt-1">{order.delivery_city}, {order.delivery_state} {order.delivery_zip}{order.equipment_make ? ` · ${order.equipment_make} ${order.equipment_model ?? ""}` : ""}</p></div>
